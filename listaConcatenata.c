...
typedef struct key
{
    char codice[50]; // Chiave di ricerca
} key;
...
nodo *list_search(nodo *head, key k);
...
La funzione di ricerca riceve in input un puntatore al primo nodo della lista 
e una chiave, che rappresenta il valore da trovare. 
